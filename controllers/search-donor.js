...
router.post('/', (req, res) => {
        ...
        db.query("SELECT * FROM user_info where b_g=? AND email != ?", [bg,email], async(err, results)=> {
            if (err) {
                ...
                if (results.length) {
                    ...
                    console.log('rokto');
                    return res.render('search-donor.ejs', { message: "No Matched Data Found",data :data})
                }
            }
        })
    }
    ...
    {
        b.query("SELECT * FROM `user_info` where division=? AND district=? AND city=? AND email!=?", [ division,district,city,email], async (err, results)=> {
            if (err) {
                throw err
            }
            else {
